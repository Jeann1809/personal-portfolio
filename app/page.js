           ...
           <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
             {projects.map((project, index) => (
               ...
               >
                 ...
                 {/* Subtle hover hint - top-right */}
                 <div className="hidden md:flex absolute top-3 right-3 items-center space-x-1 bg-white/70 text-charcoal px-2 py-1 rounded-full border border-white/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 backdrop-blur-sm">
                   <svg className="w-3 h-3 text-charcoal" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                     <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                   </svg>
                   <span className="text-xs font-sans">See details</span>
                 </div>
 
                 {/* Desktop corner indicator removed in favor of subtle pill above */}
